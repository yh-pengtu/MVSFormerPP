...
if __name__ == '__main__':
    ...
    noisy_folder = data_root.replace('Clips', 'Noisy')
    if not os.path.exists(noisy_folder): os.makedirs(noisy_folder, exist_ok=True)

    sections = glob(os.path.join(data_root, 'Sec005*'))
    data_bar = tqdm(sections, total=len(sections))
    for idx, sec in enumerate(data_bar):
        # colmap_path = os.path.join(sec, 'colmap/back/sfm')
        image_dir = os.path.join(sec, 'image_undistortion')  # origin images' path
        model_dir = os.path.join(sec, 'colmap/back/sfm/refined_geo')  # colmap path
        cam_dir = os.path.join(sec, 'mvs/back/cams')  # the path you want to save cameras
        renamed_dir = os.path.join(sec, 'mvs/back/images')  # the path you want to resave images with new order
        
        # os.makedirs(cam_dir, exist_ok=True)
        # if len(os.listdir(cam_dir)) == len(os.listdir(os.path.join(image_dir, 'back'))):
        #     continue
        ...
        # pair_path = os.path.join(sec, 'mvs/back/pair.txt')
        ...
        '''
        if not os.path.exists(renamed_dir.replace('images', 'masks')): 
            os.makedirs(renamed_dir.replace('images', 'masks'), exist_ok=True)

        if not os.path.exists(renamed_dir.replace('images', 'depths')): 
            os.makedirs(renamed_dir.replace('images', 'depths'), exist_ok=True)

        for i in range(num_images):
            v = idx_list[i]
            shutil.copyfile(os.path.join(image_dir.replace('image_undistortion', 'masks/back/foreground'), \
                images[v].name+'.png'), os.path.join(renamed_dir.replace('images', 'masks'), '%08d.png' % i))
            
            shutil.copyfile(os.path.join(image_dir.replace('image_undistortion', 'depths/back'), \
                images[v].name.replace('.jpg', '.pfm')), \
                os.path.join(renamed_dir.replace('images', 'depths'), '%08d.pfm' % i))
        '''
        ...
        for i in range(num_images):
            ...
            if abs(depth_max - depth_min) < 1:
                save_folder = sec.replace('Clips', 'Noisy')
                cmd = f'mv {sec} {save_folder}'
                os.system(cmd)